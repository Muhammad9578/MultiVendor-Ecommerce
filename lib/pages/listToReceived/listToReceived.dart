...
import 'package:mos_beauty/pages/listToReceived/services/receivedModel.dart';
import 'package:mos_beauty/pages/return/return2_page.dart';
...
class ListToReceived extends StatefulWidget {
  @override
  _ListToReceivedState createState() => _ListToReceivedState();
}

class _ListToReceivedState extends State<ListToReceived> {
  bool loading = true;
  ...
  showAlertDialogTwoButton(BuildContext context, title, okPressed, noPressed) {
    // set up the buttons
    Widget okButton = FlatButton(
      child: Text("Yes", style: TextStyle(color: HexColor('#75DFFF'))),
      onPressed: okPressed,
    );
  ...
  void _receivedOrder(PurchasesData item) {
    var jsons = {};
    jsons['purchases_id'] = item.purchasesId;

    showAlertDialogTwoButton(context, 'This item already receive?', () {
      Navigator.of(context).pop();
      ReceivedModel.purchasesReceivedOrderPhp(jsons, context);
    }, () {
      Navigator.of(context).pop();
    });
  }
  ...
  seeReturn(itemType, ListItem childItem) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (BuildContext context) => ReturnPage2(
          childItem: childItem,
        ),
      ),
    );
