...
import 'package:mos_beauty/pages/listProduct/service/discountProduct.dart';
...
import 'package:mos_beauty/pages/productDetails/productDetails.dart';
import 'package:mos_beauty/utils/format.dart';

class ListProduct extends StatefulWidget {
  final List productFromListOrder;
  final String productCategoryId;
  ListProduct({@required this.productFromListOrder, this.productCategoryId});
  @override
...
class _ListProductState extends State<ListProduct> {
  ...
  List<DiscountProductData> discountProduct = [];
  bool loading = true;
  ...
  void initState() {
    var jsons = {};
    if (widget.productCategoryId == null) {
      jsons["typeProduct"] = "product";
      jsons["isFromCategory"] = false;
    } else {
      jsons["typeProduct"] = "product";
      jsons["product_category_id"] = widget.productCategoryId;
      jsons["isFromCategory"] = true;
    }
    ListProductModel.listProductPhp(jsons, context).then((value) {
      if (value.isNotEmpty) {
        DiscountProduct.discountProductPhp(jsons).then((value2) {
          if (this.mounted) {
            setState(() {
              listProduct = value;
              discountProduct = value2;
              if (widget.productFromListOrder.isNotEmpty) {
                pushPageFromProductOrder(discountProduct);
              } else {
                loading = false;
              }
            });
  ...
  pushPageFromProductOrder(getDataDiscount) {
    if (getDataDiscount != null) {
      listProduct.forEach((item) {
        if (item.productId == widget.productFromListOrder[0]) {
          double discount = 0;
          for (var u in discountProduct) {
            if (u.itemId == widget.productFromListOrder[0]) {
              discount = double.parse(u.discountOff);
              break;
            }
          }
          double totalDiscount = double.parse(item.price) -
              (double.parse(item.price) * discount / 100);
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (BuildContext context) =>
                  ProductDetails(item, discount, totalDiscount),
            ),
          ).then((value) {
            setState(() {
              loading = false;
      ...
      listProduct.forEach((item) {
        if (item.productId == widget.productFromListOrder[0]) {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (BuildContext context) => ProductDetails(item, 0, 0),
            ),
          ).then((value) {
            setState(() {
              loading = false;
        ...
        title: Container(
          ...
          child: TextField(
            decoration: new InputDecoration(
              contentPadding: EdgeInsets.only(top: 19, left: 8),
